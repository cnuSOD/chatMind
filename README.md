...
一个本地运行的语音对话系统：平时是温暖的心理陪伴助手，需要时通过**一场自然的聊天**完成 HAMD（汉密尔顿抑郁量表）。
...
本仓库由 [cnuSOD/chatMind](https://github.com/cnuSOD/chatMind) 的语音教程合集重构而来。原有的组件演示脚本（`0~14_*.py`）、抑郁评测旧版（`15.x_*.py`、`hamd_evaluator(_v3).py`）及上游 CosyVoice 骨架（`cosyvoice/`、`webui.py` 等）均已移除，相关功能已并入 `chatmind/` 包；如需查阅可从 git 历史找回。完整变更记录见 `优化说明_代码与提示词工程.md`。
