...
# hamd_trigger_keywords = ["抑郁评估", "心理测试", "抑郁测试", "心理评估"]
...
#         model_name=model_name,  # 使用相同的语言模型
...
#     for keyword in hamd_trigger_keywords:
...
#     intro_text = "好的，我将为您进行哈密尔顿抑郁量表评估。这个评估包含16个问题，用于了解您的心理健康状况。请如实回答每个问题，这将有助于更好地了解您的情况。"
...
#                     # 当前处于HAMD评估模式，处理评估回答
#                     print(f"HAMD评估模式 - 用户回答: {prompt_tmp}")
#                     process_hamd_answer(prompt_tmp)
#                     return  # HAMD模式下直接返回，不进行正常对话
                    
#                 # --- 检查是否触发HAMD评估 ---
#                 if check_hamd_trigger(prompt_tmp):
#                     print("检测到HAMD评估触发词，开始心理评估")
...
#             if hamd_evaluator:
#                 text += "我还可以为您提供专业的心理健康评估服务，只需说出'心理评估'即可开始哈密尔顿抑郁量表测试。"
#             system_introduction(text)
...
def system_introduction(text):
    ...
    text = text
    print("LLM output:", text)
    used_speaker = "zh-CN-YunxiNeural"
    asyncio.run(amain(text, used_speaker, os.path.join(folder_path,f"sft_tmp_{audio_file_count}.mp3")))
    play_audio(f'{folder_path}/sft_tmp_{audio_file_count}.mp3')
    ...
    if flag_sv_used and is_folder_empty(set_SV_enroll):
        ...
        if flag_KWS:
            ...
            if sv_result == "yes":
                ...
                # --- 检查是否触发HAMD评估 ---
                # 使用LLM意图识别是否要开始评估
                start_intent = classify_hamd_intent(prompt_tmp)
                si = str(start_intent.get("intent", "")).lower()
                if si in ("start", "restart", "resume") or check_hamd_trigger(prompt_tmp):
                    print("检测到HAMD评估启动意图，开始心理评估")
                    start_hamd_evaluation()
                    return  # 开始评估后直接返回
